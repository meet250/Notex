...
export function Item({
  id, label, onClick, icon: Icon,
  active, documentIcon, isSearch, level = 0,
  onExpand, expanded, textColor = "text-foreground" // 👈 DEFAULT
}: ItemProps) {
    ...
    >
      {!!id && (
        <div
          ...
          role="button"
        >
          <ChevronIcon className={cn("w-4 h-4 shrink-0", textColor)} />
        </div>
      )}

      {documentIcon ? (
        <div className={cn("shrink-0 mr-2 text-[18px]", textColor)}>
          {documentIcon}
        </div>
      ) : (
        <Icon className={cn("shrink-0 w-[18px] h-[18px] mr-2", textColor)} />
      )}

      <span className={cn("truncate", textColor)}>{label}</span>

      {isSearch && (
        <kbd className={cn(
          "ml-auto pointer-events-none inline-flex gap-1 items-center h-5 select-none rounded border bg-muted px-1.5 font-mono text-[10px] font-medium opacity-100",
          textColor
        )}>
          <span className={textColor}>⌘</span>K
        </kbd>
      )}
      ...
      {!!id && (
        <div className="ml-auto flex items-center gap-x-2">
          <DropdownMenu>
            <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
              <div className="opacity-0 group-hover:opacity-100 h-full ml-auto rounded-sm hover:bg-neutral-300 dark:hover:bg-neutral-600" role="button">
                <MoreHorizontal className={cn("w-4 h-4", textColor)} />
              </div>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-60" align="start" side="right" forceMount>
              <DropdownMenuItem onClick={onArchive}>
                <Trash className={cn("w-4 h-4 mr-2", textColor)} />
                <span className={textColor}>Delete</span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <div className={cn("text-xs p-2", textColor)}>
                Last edited by: {user?.fullName}
              </div>
            </DropdownMenuContent>
          </DropdownMenu>
          <div
            ...
            onClick={onCreate}
          >
            <Plus className={cn("w-4 h-4", textColor)} />
          </div>
        </div>
      )}
    </div>
